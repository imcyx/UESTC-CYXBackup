...
def main():
    ...
    k = 11      # 可以修改此参数，看效果
    ada_classifier, _ = bagging.bagging_train(x_train, y_train, k)
    y_hat = bagging.bagging_predict(x_train, ada_classifier)
    print('Bagging算法在训练集上的分类正确率：{:.2%}'.format(np.mean(y_hat == y_train)))
    predict_y = bagging.bagging_predict(x_test, ada_classifier)
    print('Bagging算法在测试集上的分类正确率：{:.2%}'.format(np.mean(predict_y == y_test)))
