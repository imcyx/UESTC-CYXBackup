...
def generate_data():
    ...
    # 随机产生数据集
    x = np.row_stack((np.random.randn(n, 2), np.random.randn(n, 2) + 4, np.random.randn(n, 2) - 4))
    # 添加5个随机维
    x_data = np.column_stack((x, np.random.randn(len(x), 5)))

    # 仅用于绘制的标签
    y_data = np.row_stack((np.zeros((n, 1)), np.ones((n, 1)), 2 * np.ones((n, 1))))
