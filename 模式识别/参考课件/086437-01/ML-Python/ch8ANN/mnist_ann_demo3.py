...
def data_iter(x, y, batch_size=16):
    """
    获取小批量数据
    输入
        x：数据集特征
        ...
        batch_size：批量大小
    输出
        mini_batch_x，mini_batch_y：小批量的数据集特征和标签
    """
    n = len(x)
    indices = list(range(n))
    # random.shuffle(indices)  # 随机置乱
    for i in range(0, n, batch_size):
        mini_batch_idx = np.array(indices[i: min(i + batch_size, n)])  # 最后一次可能不足一个mini_batch
        yield x[mini_batch_idx], y[mini_batch_idx]
...
def main():
    # 超参数
    num_epochs = 6      # 训练模型一共需要num_epochs个迭代周期
    batch_size = 32      # 批量大小
