...
"""
多变量线性回归
使用小批量梯度下降技术，线性拟合UCI数据集——Relative CPU Performance Data
...
def minibatch_gradient_descent(x, y, theta, alpha, epochs, batch_size):
    ...
    梯度下降函数，找到合适的参数
    输入参数
        x：输入，y：输出，theta：参数，alpha：学习率，epochs：迭代轮数，batch_size：批大小
    输出参数
    ...
    n = len(y)  # 训练样本数
    batch = n // batch_size
    j_history = np.zeros((epochs * batch,))
    ...
    for epoch in range(epochs):
        for b in range(batch):
            ...
            # 用矩阵运算同时更新theta参数
            theta = theta - alpha / n * (np.dot(batch_x.T, np.dot(batch_x, theta) - batch_y))
            # 保存代价J
            j_history[epoch] = compute_cost(batch_x, batch_y, theta)
...
def main():
    ...
    # 调用梯度下降函数
    theta, j_history = minibatch_gradient_descent(x_data, y_data, theta, alpha, epochs, batch_size)
